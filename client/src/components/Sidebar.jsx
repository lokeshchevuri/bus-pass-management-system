...
const Sidebar = () => {
  ...
  const adminLinks = [
    ...
    { to: "/admin/applications", label: "Applications", icon: <FileCheck size={18} /> },
    { to: "/admin/users", label: "Manage Users", icon: <Users size={18} /> },
    { to: "/admin/reports", label: "Analytics & Reports", icon: <BarChart3 size={18} /> }
  ];
  ...
  return (
    ...
    >
      <div style={{ padding: "0 0.75rem 1rem", fontSize: "0.75rem", fontWeight: 700, color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.08em" }}>
        {isAdmin ? "Admin Navigation" : "Student Menu"}
      </div>
      ...
      {links.map((link) => (
        <NavLink
          ...
          style={({ isActive }) => ({
            ...
            background: isActive ? "linear-gradient(135deg, rgba(79, 70, 229, 0.3) 0%, rgba(6, 182, 212, 0.2) 100%)" : "transparent",
